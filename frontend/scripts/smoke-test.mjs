const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5810';
...
const adminCookie = await login('admin@newsletter-community-operations.local', 'admin123');
const managerCookie = await login('manager@newsletter-community-operations.local', 'manager123');
const analystCookie = await login('analyst@newsletter-community-operations.local', 'analyst123');
